...
from sqlalchemy import (
    ...
    ForeignKey, Enum, ForeignKeyConstraint, Index, DECIMAL
)
...
# ── 1) DEFINIMOS CoreValue ANTES QUE PlayerAssessments ────────────────────
class CoreValue(Base):
    __tablename__  = "core_values"
    __table_args__ = {"mysql_temporary": True}

    id          = Column(Integer, primary_key=True)
    name        = Column(String(255), nullable=False)
    description = Column(Text)

    # back-populates en PlayerAssessments
    assessments = relationship(
        "PlayerAssessments",    # nombre exacto de la clase
        back_populates="core_value"
    )

# ── 2) DEFINIMOS Programs ─────────────────────────────────────────────────
class Programs(Base):
...
class PlayerAssessments(Base):
    ...
    category        = Column(Enum("Técnico","Físico","Mental","Táctico","Colectivo"), nullable=False)
    core_value_id   = Column(Integer, ForeignKey("core_values.id"))
    ...
    # relaciones → nombres de clase exactos
    ...
    # tu relación a Players
    player     = relationship("Players", back_populates="player_assessments")
    coach      = relationship("Users",   backref="coach_assessments")
